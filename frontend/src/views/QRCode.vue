...
const buildBrowserPrintHtml = (items, { autoprint = true } = {}) => {
  ...
  const cards = items.map(buildLabelCardHtml)
  const pages = []
  const emptyCard = '<section class="print-label print-label--blank" aria-hidden="true"></section>'

  for (let index = 0; index < cards.length; index += 2) {
    const leftCard = cards[index] || emptyCard
    const rightCard = cards[index + 1] || emptyCard
    pages.push(`
      <section class="print-page">
        <div class="print-page__row">
          ${leftCard}
          ${rightCard}
        </div>
      </section>
  ...
  <html lang="zh-CN">
      ...
      <style>
        ...
        * {
        ...
        .print-page__row {
          ...
          transform: translateX(35mm) rotate(90deg);
        }
        ...
        .print-label__main {
          display: grid;
          grid-template-columns: minmax(0, 1fr) 13.2mm;
          gap: 1mm;
          align-items: stretch;
          flex: 1 1 auto;
          min-height: 0;
        ...
        .print-label__row {
          display: grid;
          grid-template-columns: 12.8mm minmax(0, 1fr);
          gap: 0.4mm;
        ...
        .print-label__label-group {
          display: grid;
          grid-template-columns: 11mm 1.6mm;
          align-items: baseline;
        }
    ...
    <body>
      <main class="print-sheet">${pageHtml}</main>
      ${autoprint ? `<script>
...
const showBrowserPrintSettingTip = () => {
  ...
  if (shouldShow) {
    ElMessage.info('浏览器打印请在“更多设置”里关闭页眉和页脚，边距选“无”，缩放选“实际大小/100%”。双联标签纸请设为 101mm x 35mm（纵向），并保持“每页1张/不要多页合并/不要额外旋转”。')
  }
}
